...
class FileInputUploadController extends  Controller{
    ...
    public function download($id){
        ...
        if(!empty($file)){
            return response()->download(realpath(base_path('public/uploadfile/'))."/".$file->m_url,$file->m_name);
        }else{
            return "下载错误";
        }
    }
            ...
            if(! is_null($type)  && $type >0){
                if($fileinfo['error']==0){
                    ...
                    $path = $type."/".date('Ymd')."/";
                     ...
                     if(!file_exists(env('FILE_DIR').$path)){
                         Log::error(env('FILE_DIR')."$path");
                        mkdir(env('FILE_DIR').$path,0775,true);
                    }

                    $file_save_path =env('FILE_DIR').$path;
                    $height=0;
                    $width=0;
                    ...
                    //初始化处理
                    if(in_array($extname,$this->IMG_TYPE)){
                        $initPreview=["<img src='".env('APP_URL')."/uploadfile/".$media->m_url."' style=\"width:auto;height:160px;\" class=\"file-preview-image\" alt=".$media->m_name." title=".$media->m_name.">"];
                    }else{
